...
const handleFeatureCreation = (e, map, draw, formid, refid, featuresMeta) => {
    ...
    e.features.forEach(feature => {
        if (feature.geometry.type === 'Point') {
            addPointFeature(refid, feature.geometry, style, map, featuresMeta);
        } else {
            addLineOrPolygonFeature(refid, feature.geometry, style, map, featuresMeta);
        }

        const existingFeature = drawnFeatures.features.find(f => f.properties.refid === refid);
        if (existingFeature) {
            draw.delete(existingFeature.id);
        }

        draw.add({
            type: 'Feature',
            geometry: feature.geometry,
            properties: { refid, style }
        });

        fetch(`/api/v2/update_feature/${formid}/${refid}`, {
            method: 'PUT',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({ geom: feature.geometry, style })
        })
...
const handleFeatureUpdate = (e, map, formid, refid) => {
    ...
    updatedFeatures.forEach((feature) => {
        ...
        if (feature.geometry.type === 'Point') {
            if (featuresMeta[refid] && featuresMeta[refid].marker) {
                featuresMeta[refid].marker.setLngLat(feature.geometry.coordinates);
            }
        } else {
            if (map.getSource(refid)) {
